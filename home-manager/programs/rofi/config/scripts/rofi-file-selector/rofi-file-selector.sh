...
command -v wl-copy >/dev/null 2>&1 || { echo >&2 "I require 'wl-copy' but it's not installed.  Aborting."; exit 1; }
...
{ 
   ...
   | { "$_ROFI" -theme-str "#window { width: 900;}"  \
    -dmenu -sort -sorting-method fzf -matching fuzzy -i -p "Choose to open" \
